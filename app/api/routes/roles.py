...
@has_permission(Permissions.READ_ROLE)
async def get_role_by_id(role_id:int, current_user: CurrentUser, db: Session = Depends(get_db)):
    role_repo = RoleRepository(db)
...
async def update_role(role_id:int, role_data: RoleBase, current_user: CurrentUser, db: Session = Depends(get_db)):
    role_repo = RoleRepository(db)
    return role_repo.update_role(role_id, role_data)

@router.delete('/{role_id}', status_code=status.HTTP_204_NO_CONTENT)
@has_permission(Permissions.DELETE_ROLE)
async def delete_role(role_id:int, current_user: CurrentUser, db: Session = Depends(get_db)):
    role_repo = RoleRepository(db)
